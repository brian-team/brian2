...
from .codeobject import CStandaloneCodeObject, openmp_pragma
...
class CWriter(object):
    ...
    def write(self, filename, contents):
        ...
        if filename.lower().endswith('.c'):
            ...
            self.write(filename[:-1]+'c', contents.cpp_file)
...
class CStandaloneDevice(Device):
    '''
    The `Device` used for C++ standalone simulations.
    '''
    def __init__(self):
        super(CStandaloneDevice, self).__init__()
    ...
    def code_object_class(self, codeobj_class=None):
        ...
        return CStandaloneCodeObject

    def code_object(self, owner, name, abstract_code, variables, template_name,
                    variable_indices, codeobj_class=None, template_kwds=None,
                    override_conditional_write=None):
        codeobj = super(CStandaloneDevice, self).code_object(owner, name, abstract_code, variables,
    ...
    def build(self, directory='output',
        ...
        for net in networks:
            ...
            if len(net_synapses):
                raise NotImplementedError('C standalone does not support synapses')
            if any(isinstance(obj, SpikeMonitor) for obj in net.objects):
                logger.warn('SpikeMonitor will only record total spike count',
                            once=True)
        ...
        for codeobj in self.code_objects.itervalues():
            ...
            for k, v in codeobj.variables.iteritems():
                if isinstance(v, AttributeVariable):
                    ...
                    line = 'const {c_type} {varname} = {typename}_{attrname}({objname});'
                    lines.append(line.format(c_type=c_data_type(v.dtype), varname=k,
                                             typename=v.obj.__class__.__name__,
                                             objname=v.obj.name,
        ...
        for codeobj in self.code_objects.itervalues():
            ...
            writer.write('code_objects/'+codeobj.name+'.c', code)
        ...
        # smarter about that.
        main_tmp = CStandaloneCodeObject.templater.main(None, None,
        ...
        run_tmp = CStandaloneCodeObject.templater.run(None, None, run_funcs=runfuncs,
                                                        code_objects=self.code_objects.values(),
        ...
        brianlib_dir = os.path.join(os.path.split(inspect.getsourcefile(CStandaloneCodeObject))[0],
                                    'brianlib')
        ...
        if compiler=='msvc':
            ...
            source_bases = [fname.replace('.c', '').replace('/', '\\') for fname in writer.source_files]
            win_makefile_tmp = CStandaloneCodeObject.templater.win_makefile(
                None, None,
            ...
            makefile_tmp = CStandaloneCodeObject.templater.makefile(None, None,
...
class RunFunctionContext(object):
    ...
    def __enter__(self):
        c_standalone_device.main_queue.append(('start_run_func', (self.name, self.include_in_parent)))
    def __exit__(self, type, value, traceback):
        c_standalone_device.main_queue.append(('end_run_func', (self.name, self.include_in_parent)))

c_standalone_device = CStandaloneDevice()
all_devices['c_standalone'] = c_standalone_device


class CStandaloneSimpleDevice(CStandaloneDevice):
    def network_run(self, net, duration, report=None, report_period=10*second,
                    namespace=None, profile=True, level=0, **kwds):
        super(CStandaloneSimpleDevice, self).network_run(net, duration,
                                                     report=report,
...
c_standalone_simple_device = CStandaloneSimpleDevice()

all_devices['c_standalone_simple'] = c_standalone_simple_device
