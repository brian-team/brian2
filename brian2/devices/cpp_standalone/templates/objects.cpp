...
void _init_arrays()
{
    ...
    array_meta_data.insert({"{{var.owner.name}}.{{var.name}}", { false, {{var.size}}, "{{c_data_type(var.dtype)}}", {{varname}} } });
    {% endif %}
    {% if var in dynamic_array_specs and not var.read_only %}
    {# size will be directly requested from vector #}
    array_meta_data.insert({"{{var.owner.name}}.{{var.name}}", { true, 0, "{{c_data_type(var.dtype)}}", &{{dynamic_array_specs[var]}} } });
    {% endif %}
	{% endfor %}
