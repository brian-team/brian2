...
class FixedStateMonitor(Group, CodeRunner):
    '''
    Record values of state variables during a run, using a static array instead
    of dynamic arrays.
    ...
    runtime : `Quantity`
        The total runtime this monitor will record.
    ...
    def __init__(self, source, variables, runtime, record=None, dt=None, clock=None,
        ...
        self.timesteps = int(np.ceil(self.runtime/clock.dt))
        ...
        CodeRunner.__init__(self, group=self, template='fixedstatemonitor',
                            code=code, name=name,
                            clock=clock,
                            dt=dt,
                            when=when,
        ...
        self.variables.add_array('t', size=self.timesteps, unit=second,
                                 constant=False)
        ...
        for varname in variables:
            ...
            self.variables.add_array('_recorded_' + varname,
                                     size=(self.timesteps, len(self.record)),
                                     unit=var.unit,
                                     dtype=var.dtype,
                                     constant=False)
