...
def test_spikequeue():
    ...
    queue = SpikeQueue()
    queue.prepare(delays[:], dt, synapses, N, N, source_start=0)
    queue.push(np.arange(N, dtype=np.int32), start=0, stop=N)
    for i in xrange(N):
    ...
    queue = SpikeQueue()
    queue.prepare(delays[:], dt, synapses, N*N, N, source_start=0)
    queue.push(np.arange(N*N, dtype=np.int32), start=0, stop=N)
    for i in xrange(N):
