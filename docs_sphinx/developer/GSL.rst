...
Our solution is to replace the y[0]s and f[0]s etc. with a 'normal' variable name
...
So far, each code generation language (numpy, weave, cython) there was just
...
already existing codeobject of its language. There are only minimal changes
to the already existing codeobject:

* Overwrite stateupate template: a new version of the stateupdate template is
  given (stateupdate.cpp for weave/cpp_standalone and stateupdate.pyx for
  cython).
...
This defining of GSL-specific codeobjects also  allowed us to catch compilation
...
the cpp CodeObject such overriding wasn't really possible so compilation
...
solvers to achieve stateupdates.
...
is generating the code that descibes the code objects. This starts with a
...
* Brian's general preparatory work. This piece of code is directly copied from
  the base CodeGenerator and is thus similar as what is done normally.
...
* Instead of referring to the variables by there name only (e.g. ``dv/dt =
...
* The values of the variables in the datastructure have to be set to the
...
* ``'int _fill_y_vector(_dataholder* _GSL_dataholder, double* _GSL_y, int _idx)'``':
  pulls out the values for each differential variable out of the 'Brian' array into the y-vector. This happens in the vector loop (e.g. y[0] = _GSL_dataholder->_ptr_array_neurongroup_v[_idx]; for weave/cpp).
