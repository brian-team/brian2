...
compartment_start=morpho.axon[Na_start].indices()[0]
compartment_end=morpho.axon[Na_end].indices()[0]
neuron.v=EL
neuron.gclamp[0]=gL*500

if linear_distribution:
    profile=linspace(1,0,compartment_end-compartment_start)
else:
    profile=ones(compartment_end-compartment_start)
profile=profile/sum(profile) # normalization

neuron.gNa[compartment_start:compartment_end]=gNa*profile/neuron.area[compartment_start]
